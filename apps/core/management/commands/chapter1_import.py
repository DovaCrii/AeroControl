...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--workbook",
            type=Path,
            help="Excel workbook with cost_centers, aircraft and operators sheets.",
        )
    ...
    def parse_file(self, key, path):
        ...
        if any(not value for value in values) or len(values) != len(set(values)):
            raise CommandError(
                f"{key} contains empty or duplicate {unique_field} values"
            )
        model = SCHEMAS[key][0]
        ...
        if key in {"aircraft", "operators"}:
            for row in rows:
                if not CostCenter.objects.filter(
                    code=row["cost_center"], is_active=True
                ).exists():
                    raise CommandError(
                        f"{key} references unknown cost center {row['cost_center']}"
                    )
        if key == "aircraft":
            for row in rows:
    ...
    def parse_workbook(self, path):
        ...
        for key, sheet_name in sheet_names.items():
            ...
            if headers != expected:
                raise CommandError(
                    f"{sheet_name} columns must be: {','.join(expected)}"
                )
            rows = []
    ...
    def handle(self, *args, **options):
        ...
        payload = (
            self.parse_workbook(options["workbook"])
            if options.get("workbook")
            else {key: self.parse_file(key, path) for key, path in paths.items()}
        )
        if options.get("workbook"):
            for key, rows in payload.items():
                ...
                if key in {"aircraft", "operators"}:
                    for row in rows:
                        if not CostCenter.objects.filter(
                            code=row.get("cost_center"), is_active=True
                        ).exists():
                            raise CommandError(
                                f"{key} references unknown cost center {row.get('cost_center')}"
                            )
        result = {
            "mapping": "chapter1-v1",
            "apply": options["apply"],
            "rows": {key: len(rows) for key, rows in payload.items()},
        }
        if options["apply"]:
            with transaction.atomic():
                for key, rows in payload.items():
                    ...
                    batch = ImportBatch.objects.create(
                        actor=None, entity=f"chapter1.{key}", rows=rows
                    )
                    created = []
                    for row in rows:
                        if key == "aircraft":
                            row["cost_center_id"] = CostCenter.objects.get(
                                code=row.pop("cost_center")
                            ).pk
                        elif key == "operators":
                            row["cost_center_id"] = CostCenter.objects.get(
                                code=row.pop("cost_center")
