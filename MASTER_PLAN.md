...
| LV-59 | ⬜ | P3 | **Pendiente** (pedido del usuario, sin capturas concretas): revisar el **diseño/metodología de trabajo del módulo Vuelos** (`FlightRecord`/`/operations/records/`) y cómo mejorarlo — el nombre del módulo, su flujo, y qué cambiaría. **No definido aún**: falta que el usuario aporte capturas o ejemplos concretos de qué le molesta del flujo actual (mismo patrón "capturas primero" de LV-53/57/58), antes de proponer un diseño. | `apps/operations/` (a confirmar) |
| LV-60 | ⬜ | P2 | **Pendiente** (pedido del usuario con captura de "Importar KMZ/KML"): **unificar Planificación geoespacial con Permisos de vuelo** — hoy son dos módulos separados que en la práctica son **una sola etapa** del mismo proceso (solicitar un permiso = permiso + documentación + el plan KMZ), y el usuario nota que se siente redundante pedir un **nuevo título** al importar un plan cuando ya se está vinculando a un permiso de vuelo que **ya tiene** su propia identidad (folio/propósito). Idea a definir: cuando `GeoPlanImportForm` recibe `?flight_permission=<pk>` (ya prellenado desde LV-50), el título del plan podría heredar/derivar del permiso en vez de pedirse de nuevo, y a más largo plazo evaluar si "Planificación geoespacial" debería vivir como una **pestaña/etapa dentro de la ficha del permiso** en vez de un módulo aparte del sidebar. **No implementado** — es una propuesta de diseño de flujo, no un fix puntual; falta acordar el alcance exacto con el usuario (¿solo heredar el título, o fusionar los dos módulos?). | `apps/geo/forms.py`, `templates/operations/permission_detail.html`, `templates/geo/plan_import.html` (a confirmar) |
| LV-61 | ⬜ | P3 | **Pendiente** (pedido del usuario, con captura del menú lateral completo): revisar la **distribución y agrupación del menú de navegación** — nombres de sección en general y cómo agruparlos para que reflejen el flujo real de uso (no solo la estructura de datos), continuando el reordenamiento de LV-13a (2026-07-30). **No definido aún**: falta que el usuario aporte más detalle de qué agrupación/orden prefiere (¿por frecuencia de uso? ¿por etapa del proceso operativo, como ya sugiere LV-60?) antes de tocar `templates/base.html`. | `templates/base.html` (a confirmar) |
