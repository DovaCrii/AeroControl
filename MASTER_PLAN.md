...
| ID | Est. | Tarea |
|---|:--:|---|
| X.1 | 🔄 | **`Aircraft.serial_number` como llave de cruce — normalización hecha 2026-08-10, discrepancias reales pendientes de verificación física.** `Aircraft.save()` ahora limpia **todo** el whitespace (interno además de extremos — `"".join(serial.split())`, no `.strip()`) y migración de datos `0027` aplica lo mismo a las filas existentes. Confirmado contra la copia restaurada + `Z:` en vivo: las 2 discrepancias de espacio (`RPA-4401`, `RPA-4436`) eran exactamente eso — el folio de `Z:` no tiene el espacio que sí tenía la app — y ya quedan resueltas por la normalización. **Las otras 2 siguen abiertas y no son de normalización**: `RPA-4647` (app `...246B00D7WPK` con ceros, carpeta `...246BOOD7WPK` con letras "OO") y `RPA-4884` (app `1581F5FHC24BB00DMXNH`, carpeta `CC717-1582F5FHC24BB00DMXNH`) — ninguna fuente que ya tenemos (base de datos, `Z:`) puede arbitrar cuál es el serial real; hace falta la placa física del equipo o el certificado DGAC. `unique=True` queda **sin imponer** hasta resolverlas. |
