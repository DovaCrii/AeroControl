...
| LV-59 | ⬜ | P3 | **Pendiente — diagnóstico hecho con Opus 5 (2026-08-04), falta decidir el alcance con el usuario.** Pedido original: revisar el diseño/metodología del módulo **Vuelos** (`FlightRecord`, `/operations/records/`). Hallazgos concretos de la revisión del código: (1) **Todo `FlightRecord` cuelga obligatoriamente de un `FlightPermission`** (FK `PROTECT`, no nullable) — o sea, un vuelo es *la ejecución* de un permiso, nunca algo independiente; pero la lista es un módulo de primer nivel donde se parte de cero y hay que **elegir el permiso a mano** en un desplegable de todos los permisos, cuando el camino natural (y ya existente) es abrir el permiso y usar su botón "+ Agregar registro", que **ya prellena** permiso/piloto/aeronave (`FlightRecordCreate.get_initial`). (2) La lista **no tiene plantilla propia ni partial de filas**: usa `generic/list.html` con las columnas por defecto (Nombre/Creado/Estado), a diferencia de Permisos (LV-53) y las 4 del padrón (LV-57) — de ahí que se viera como "Nombre" en la captura del usuario. (3) `FlightRecord` **no declara `verbose_name`**, lo que causó el título en inglés que se corrigió aparte en LV-62. **Propuestas a decidir**: (a) normalizar la lista al patrón LV-57 con columnas reales (Fecha/Permiso/Piloto/Aeronave/Duración) — barato y alineado con lo ya hecho; (b) calcular y mostrar la **duración** del vuelo (hoy se guardan `departure_time`/`arrival_time` pero nunca se restan en ninguna vista); (c) replantear si Vuelos debe seguir siendo un módulo de primer nivel o sobre todo una pestaña del permiso (paralelo a la decisión de LV-60/61). | `apps/operations/views.py`, `templates/operations/` (a confirmar) |
