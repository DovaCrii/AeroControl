...
| T3.2 | 🔄 | P1 | Tenancy (F-08). **ADR-0001 aceptado** (Opción A, por fases — [docs/dev/adr-0001-tenancy.md](docs/dev/adr-0001-tenancy.md)). **Fase 0a hecha 2026-07-31**: `get_default_tenant` (default del FK, sin churn) + `CostCenter`/`Aircraft`/`Operator.tenant` `NOT NULL` con backfill (migr. 0018/0019). Pendiente: Fase 0b (resto de raíces; hijos derivan del padre), Fase 1 (tenant del request), Fase 2 (mixin único de scoping), Fase 3 (constraints únicas por tenant). Nota: T3.2 ya no depende de T1.1 en la práctica (se hace sin partir `core`). | XL | — |
