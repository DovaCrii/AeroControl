...
| LV-33 | ⬜ | P1 | **Pestaña Equipo/Flota del CC vacía** (revisión en vivo 2026-08-03, feedback del usuario). Operadores/aeronaves importados con `cost_center` directo no tienen `OperatorAssignment`/`AircraftAssignment`, y el detalle lee de esas tablas → pestañas vacías aunque el CC tenga 3 asignados. **Acción inmediata**: `backfill_resource_assignments` en prod (idempotente). **Código (robustez)**: que el detalle liste por `cost_center` aunque falte el assignment, para no depender del backfill tras cada import. | `registry/views.py` CostCenterDetail |
| LV-34 | ⬜ | P1 | **Rediseño del responsable del CC** (feedback del usuario): selector "Tipo de responsable" (Administrador/Operador/Externo) con campos condicionales — Administrador=solo nombre; Operador=lista; Externo=nombre+correo. JS en estático (CSP), validación/guardado por tipo. Optimiza espacio y flujo. | `CostCenterForm` + JS + plantilla |
...
| LV-36 | ⬜ | P3 | **Editar CC en pestañas** como el detalle (form plano largo → secciones/pestañas). Aclarar alcance con el usuario. | plantilla del form CC |
...
| LV-38 | ⬜ | P1 | **Form de permiso — grilla + optimizar espacio** (feedback usuario 2026-08-03; concreta **LV-23**). Operadores (~40) y flota (~14) como grilla multi-columna con scroll acotado + búsqueda, no lista vertical larga; mantener selección múltiple. Resto de campos en layout compacto (fechas lado a lado). Requiere navegador (crispy). | `FlightPermissionForm` + plantilla + `app.css` |
| LV-39 | ⬜ | P1 | **Permiso — Estado antes del número + número condicional** (feedback usuario): 'Estado' (en emisión/aceptado/rechazado) al inicio; número **no obligatorio** salvo Estado=aceptado (validación en `clean`), para armar el permiso antes del folio. | `FlightPermissionForm` |
