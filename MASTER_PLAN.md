...
| R7.7 | ✅ | 9.1.1 KPIs | **Diseño entregado 2026-08-11** — [docs/dev/iso-r7-design-plan.md](docs/dev/iso-r7-design-plan.md). **Resuelve el hallazgo que `R6.4` documentó y dejó abierto**: los 3 KPI actuales se evalúan siempre "a hoy" sin importar el período, así que su tendencia **no puede moverse por construcción** — un KPI sin tendencia posible no es un KPI. Solución concreta y autocontenida: **`ComplianceSnapshot`**, un trabajo programado que guarda los totales del día; la tendencia pasa a ser historia real en vez de dos fotos del mismo instante. Solo agrega, no toca nada, **y no necesita ninguna decisión de negocio — es la recomendación de por dónde seguir**. Además: `KpiTarget` para la meta (config por tenant con FK propio, misma naturaleza que `AlertRule` según ADR-0001), y la acción ante incumplimiento **debe reusar el tablero o `NonConformity`, no ser un tercer mecanismo**. Hallazgo útil: **2 de los 5 KPI operacionales que pide la guía ya son calculables hoy** (disponibilidad vía `Aircraft.status` + estados de taller de R5.1; cumplimiento de plazos vía fechas de permiso vs. vuelo). |
