...
| T2.2 | 🔄 | P1 | `TenantScopedQuerysetMixin` en documentos + filtro por entidad (F-05). **Parcial 2026-08-03:** `core.tenancy.scope_queryset_to_tenant` + `registry.TenantScopedQuerysetMixin` cierran el **IDOR por objeto (F-03/F-06)** en detalle/edición/archivar/restaurar de los modelos raíz (CostCenter/Aircraft/Operator con FK `tenant` directo): otro tenant recibe **404**, el dueño 200. No-op hoy (un tenant) pero correcto al centralizar. 3 tests en `test_tenancy.py::TestObjectLevelIsolation`. **Pendiente:** aplicarlo a documentos/permisos/mantenimiento y modelos derivados (scoping por relación). | M | — |
...
| LV-24 | ⬜ | P3 | **[diferido, requiere navegador]** Badges de estado inconsistentes (sólidos vs paleta *subtle* de LV-15) → armonizar. Cambio estético amplio, mejor con verificación visual. | listas varias |
| LV-25 | ⬜ | P3 | **[diferido, requiere revisión de datos]** VLOS/Paracaídas como texto libre → `ChoiceField`. Los datos actuales tienen valores libres ("VLOS", "NO"); pasar a choices exige normalizar o un choice "otro" para no rechazar registros existentes al editar. | `AircraftForm` |
