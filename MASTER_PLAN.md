...
4. **Bajo demanda del usuario, no proactivo**: ~~B3.1/B3.2/B3.5~~ hechos el
   2026-08-07 (ver BLOQUE 3 abajo); ~~T5.7~~ hecho el 2026-08-10; queda LV-6
   (vista Gantt, requiere decisión de diseño), T5.8, LV-59 opción (c) si se
   quiere reabrir, LV-65 (selector de aeronaves en Mantenciones).
...
| T5.7 | ✅ | P2 | **Exportación visible en todas las listas (U6).** Auditadas todas las `ListView` del proyecto: 6 gaps encontrados y cerrados. `ResourceMovementLogList` (registry) exporta el `resource_label` legible en vez del UUID crudo (`render_csv_response` propio, como `TaskReportCsvView`). `GeoPlanListView`, `CompanyDocumentsView` y `OperationalRecordsView` usan `CsvExportMixin` con el export genérico (mismo patrón que el resto de listas del modelo `Document`). `AuditEventListView` y `UserRoleListView` usan `csv_fields` explícito en vez del genérico: el primero porque el default excluye `created_at` (el dato central de un log de auditoría); el segundo porque el modelo `User` expone `password` y no debe salir nunca en un CSV. `KanbanTaskListView`/`MonthlyReviewView`/`CalendarView` ya tenían su propio mecanismo o no aplican (grilla agregada). 8 tests nuevos. | M | — |
