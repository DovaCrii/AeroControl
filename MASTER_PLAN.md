...
| GEO-11 | 🔄 | Capas avanzado: reorder/mover entre folders/duplicar/**explotar MultiGeometry** (split por vértice diferido). Todo cliente (`doc.js` mutadores puros + `panel.js` árbol con DnD + `edit.js` inserción folder-aware); servidor sin cambios (el commit re-valida el árbol). Diseño aprobado 2026-07-29: [docs/dev/geo-v2-plan.md](docs/dev/geo-v2-plan.md) | GEO-8 | V2 |
| GEO-12a | 🔄 | **Diff visual entre versiones** (por `uid`, 100% cliente, `diff.js` + modo diff en el shell; sin cambio de servidor). Aprobado 2026-07-29 | GEO-10 | V2 |
...
| GEO-13 | ⬜ | **[DIFERIDO]** Iconos embebidos servidos (posible add-on: endpoint que abre el KMZ fuente con los guards de `kmz.py`), StyleMap highlight editable y validación XSD (estos dos: alto riesgo de round-trip / bajo ROI, probablemente no en un editor liviano) | GEO-10 | V2 |
