...
| R3.3 | 🔄 | **Estados visibles**: (a) ✅ hecho — el operador archivado ahora muestra una insignia "Archivado" y el botón cambia a "Restaurar" en vez de "Editar" (`_operator_rows.html`), igual que la tabla genérica ya hace para modelos sin partial propio. (b) ⬜ — `CostCenter` no tiene "contrato cerrado" (solo `is_active` genérico); requiere decisión de negocio sobre semántica (¿eje distinto de archivado, o lo mismo con otro nombre?) antes de tocar el modelo. (c) ✅ **investigado 2026-08-07, ya estaba implementado** — `Aircraft.retired` ya se ve (columna "Status" en `aircraft_list.html`, insignia gris) y ya es editable (`AircraftForm` incluye `status`); solo faltaba el test de regresión, agregado. | `templates/registry/_operator_rows.html`, `apps/registry/test_r33_visible_states.py`
