...
| R4.8 | 🔄 | Tipo de documento nuevo **"Aviso Mensual de No Operación"**, más `aoc-certificate` y `company-procedure` (para R4.6) siguen sin crear. **`maintenance-certificate`, `flight-request` e `incident-investigation-record` ya se agregaron a `seed_document_types`** (adelantados desde acá el 2026-08-10, R4.1 no podía clasificar las subcarpetas `02.-`/`03.-`/`04.-` de `Z:` sin ellos), y **`calibration-certificate` se sumó el 2026-08-11 (R7.3)** — **gotcha de despliegue LV-45/LV-64 sigue vigente: correr `seed_document_types` a mano en `p340`** (pasa de 10 a 14 tipos). |
