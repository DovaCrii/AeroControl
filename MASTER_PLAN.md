...
| LV-57 | ⬜ | P2 | **Pendiente** (pedido del usuario con 4 capturas: listas de Centros de costo, Aeronaves, Operadores y Habilitaciones): las columnas y su distribución/ancho no están normalizadas entre estas 4 listas — cada una reparte el espacio distinto (p. ej. "Nombre de administrador de contrato" ocupa una franja ancha en Centros de costo mientras columnas numéricas quedan angostas), el patrón de columna primaria (matrícula+modelo en Aeronaves) no se replica en las demás, y la acción de fila dice "Editar" en 3 listas pero "Ver operador" en Habilitaciones. El usuario pide normalizar el mismo estilo en las 4 y aprovechar mejor el espacio — continúa el trabajo de **LV-32** (sistema de tablas general, quedó parcial) ahora con capturas concretas de qué módulos tocar. **No implementado aún** — falta acordar con el usuario el patrón exacto (columna primaria en todas, ancho proporcional, verbo de acción único) antes de tocar las 4 plantillas. | `templates/registry/_costcenter_rows.html`, `_aircraft_rows.html`, `_operator_rows.html`, `_qualification_rows.html` (a confirmar) |
