...
| T5.6 | 🔄 | P2 | **Paginación HTMX + búsqueda en vivo alineadas (F-13).** Diagnóstico 2026-08-03: son **dos** problemas. (a) La búsqueda/paginación en vivo swapea solo `#table-body` (el `<tbody>`), pero los controles de paginación viven fuera → quedan **stale**. (b) Solo `DocumentList` sobrescribe `htmx_template_name`; el resto de listas custom devuelven el `_table_body.html` genérico en HTMX, así que al buscar sus **columnas custom colapsan** a las genéricas (thead de N vs filas de 3 celdas). **(a) resuelto** con out-of-band swap de la paginación (testeable a nivel de respuesta). **(b) pendiente**: requiere un parcial de filas por lista (como `_document_rows.html`) y **verificación en navegador** — dejar para sesión con login disponible. | S→M | — |
