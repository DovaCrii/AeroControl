...
| LV-16b | ✅ | P3 | **Nombrar CC nuevos = decisión: se hace por `/admin`** (2026-07-31). LV-16 sacó `name` del `CostCenterForm` a propósito; al pedir "CC110 Casa Matriz" el usuario eligió **mantener LV-16** y crear los CC con nombre vía `/admin/registry/costcenter/add/` (superusuario), no reincorporar el campo. `CostCenter.name` sigue `blank=True`. Casa Matriz creada en local; en prod la crea el usuario por `/admin`. | — |
